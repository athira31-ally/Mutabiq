...
## 7. Open risks — resolve before writing code

1. **Exact Trakheesi/Madhmoun permit-number format is unconfirmed.** Published guidance describes the *rule* (permit number must appear and match) but not the literal format (digit count, prefix, checksum). Plan: pull the format directly from 10–20 real permit numbers in the seed set rather than guessing a regex up front.
2. **No confirmed public permit-lookup/verification API.** PropSpace's own write-up notes portals run their own Trakheesi check and bear final responsibility — implying no guaranteed public real-time verification endpoint. v1 should assume format + cross-check validation only, not live DLD verification, unless one is found.
3. **Scraping ToS.** Property Finder/Bayut listing pages are public, but automated scraping at any scale needs a ToS check first. v1 seed set should be collected manually/in small volume, not via an unrestricted scraper.

## 8. Milestone plan (~2–3 weeks)

- **Week 1:** Manual seed-data collection (~100–200 listings) → confirm permit format → build OCR + regex + cross-check module → rule-engine skeleton.
- **Week 2:** Synthetic watermark dataset → fine-tune YOLOv8n → export ONNX → duplicate-photo hashing module.
- **Week 3:** FastAPI service, ONNX Runtime integration, deploy to Azure, write up results (precision/recall numbers) in the README, update the portfolio site status badge to 🟢.
